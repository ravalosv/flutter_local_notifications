...
class LocalNotificationService {
  ...
  factory LocalNotificationService.obtenerInstancia() {
    // Si la instancia aún no se ha creado, la crea.
    _instancia ??= _instancia = LocalNotificationService._();
    // Devuelve la instancia existente o recién creada.
    return _instancia!;
  }

  // Declaración de una variable privada estática que contendrá la única instancia de la clase.
  static LocalNotificationService? _instancia;

  // Constructor factory privado para evitar la creación de instancias fuera de la clase.
  ...
  Future<int> initializeLocalNotificationService() async {
  ...
  Future<void> showBigPictureNotification() async {
    final String largeIconPath = await downloadAndSaveFile('https://dummyimage.com/48x48', 'largeIcon');
    final String bigPicturePath = await downloadAndSaveFile('https://dummyimage.com/400x800', 'bigPicture');
  ...
  Future<void> showBigPictureNotificationURL() async {
    final ByteArrayAndroidBitmap largeIcon = ByteArrayAndroidBitmap(await getByteArrayFromUrl('https://dummyimage.com/48x48'));
    final ByteArrayAndroidBitmap bigPicture = ByteArrayAndroidBitmap(await getByteArrayFromUrl('https://dummyimage.com/400x800'));
  ...
  Future<void> showNotificationMediaStyle() async {
    final String largeIconPath = await downloadAndSaveFile('https://dummyimage.com/128x128/00FF00/000000', 'largeIcon');
  ...
  Future<void> showMessagingNotification() async {
    ...
    final Person chef = Person(name: 'Master Chef', key: '3', uri: 'tel:111222333444', icon: ByteArrayAndroidIcon.fromBase64String(await base64encodedImage('https://placekitten.com/48/48')));
  ...
  Future<void> showNotificationWithAttachment({
    ...
    final String bigPicturePath = await downloadAndSaveFile('https://dummyimage.com/600x200', 'bigPicture.jpg');
    final DarwinNotificationDetails darwinNotificationDetails = DarwinNotificationDetails(
  ...
  Future<void> getActiveNotifications(BuildContext context) async {
    final Widget activeNotificationsDialogContent = await getActiveNotificationsDialogContent(context);
    await showDialog<void>(
      context: context,
      builder: (BuildContext context) => AlertDialog(
                ...
                children: <Widget>[
                  ...
                  if (Platform.isAndroid && activeNotification.id != null)
                    TextButton(
                      ...
                      onPressed: () {
                        getActiveNotificationMessagingStyle(context, activeNotification.id!, activeNotification.tag);
                      },
                    ),
                  const Divider(color: Colors.black),
                ],
  ...
  Future<void> getActiveNotificationMessagingStyle(BuildContext context, int id, String? tag) async {
